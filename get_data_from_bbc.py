...
if __name__ == '__main__':
    ...
    constituencies = pd.DataFrame(columns=['name', 'region', 'region_code', 'outcome', 'majority', 'registered', 'turnout', 'turnout_delta'])
    i = 0
    ...
    for j, url in enumerate(constituency_urls):
        ...
        region_code = url.split('/')[-1]

        turnout_stats = [x.text for x in soup.find_all("span", {"class": "ge2019-constituency-result-turnout__value"})]
        constituencies.loc[j] = [constituency_name, region, region_code, outcome, *turnout_stats]
